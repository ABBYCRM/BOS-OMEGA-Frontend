  ...
  const onSubmit = (e: FormEvent) => {
    ...
    if (mode === "signup") {
      if (password.length < 8) {
        setError("CLEARANCE CODE BELOW MINIMUM ENTROPY (8 CHARS).");
        flashGlitch();
        return;
      }
      if (password !== confirm) {
        setError("CLEARANCE CODES DO NOT MATCH. RE-ENTER.");
        flashGlitch();
        return;
      }
    ...
    <div className="min-h-screen w-full overflow-hidden relative font-mono text-[#e8e6e1] bg-black">
      ...
      <div className="relative z-10 flex items-center justify-between px-6 py-3 text-[10px] tracking-[0.25em] text-[#7a6e6c] border-b border-[#1a0608]">
        <div className="flex items-center gap-3">
          <span className="inline-block w-2 h-2 rounded-full bg-[#dc1e28]" style={{ animation: "umbrella-blink 1.4s steps(2,end) infinite" }} />
          <span>SECURE LINK · TLS 1.3 · AES-256-GCM</span>
        </div>
        <div className="flex items-center gap-6">
          <span>TERMINAL · {terminalId}</span>
          <span style={{ animation: "umbrella-flicker 5s infinite" }}>RED QUEEN ONLINE</span>
        </div>
      </div>
          ...
          >
            ...
            >
              <div className="relative mb-4">
                <div
                  ...
                  style={{ background: "radial-gradient(circle, rgba(220,30,40,0.45) 0%, transparent 65%)" }}
                />
            ...
            <form onSubmit={onSubmit} className="px-6 pt-5 pb-6 space-y-4">
              ...
              <Field
                ...
                placeholder={mode === "signup" ? "MIN 8 CHARS" : "ENTER CLEARANCE"}
                type="password"
                value={password}
              ...
              <button
                type="submit"
                disabled={isPending || !email || !password || (mode === "signup" && !confirm)}
      ...
      {/* Bottom legal strip */}
      <div className="relative z-10 px-6 py-2 text-[9px] tracking-[0.3em] text-[#5a4e4c] border-t border-[#1a0608] flex justify-between">
...
function UmbrellaLogo({ size = 80 }: { size?: number }) {
  ...
  return (
    ...
    >
      <circle cx={cx} cy={cy} r={45} fill="#0c0304" stroke="#3a1116" strokeWidth="1.5" />
      <g>{wedges}</g>
      <circle cx={cx} cy={cy} r={6} fill="#0c0304" stroke="#3a1116" strokeWidth="1" />
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 100 100" className={className} fill="currentColor" aria-hidden>
      <g>
        ...
        <path d="M81 70 a16 16 0 0 1 -27.72 0 L60 58 z" transform="rotate(120 50 50)" />
        <path d="M81 70 a16 16 0 0 1 -27.72 0 L60 58 z" transform="rotate(240 50 50)" />
      </g>
    </svg>
  );
}
