...
export function ModelRegistry() {
  ...
  const tagColors: Record<string, string> = {
    reasoning: "bg-blue-50 text-blue-800 border-blue-200",
    coding: "bg-violet-50 text-violet-800 border-violet-200",
    long_context: "bg-cyan-500/20 text-cyan-300 border-cyan-500/30",
    cheap: "bg-green-50 text-green-800 border-green-200",
    fast: "bg-yellow-500/20 text-yellow-300 border-yellow-500/30",
    structured_output: "bg-orange-500/20 text-orange-800 border-orange-200",
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-serif font-semibold text-foreground tracking-tight">Model registry</h1>
                ...
                <button
                  ...
                  className={`px-2 py-0.5 rounded border text-[11px] font-mono transition-all ${
                    newModel.capability_tags.includes(tag)
                      ? tagColors[tag] || "bg-secondary text-primary border-border"
        ...
        <table className="w-full text-xs font-mono">
          ...
          <tbody>
            {isLoading ? (
              ...
              models.map((m) => (
                <tr key={m.id} className="border-b border-border/50 hover:bg-secondary transition-colors">
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex flex-wrap gap-1">
                      {(m.capability_tags || []).slice(0, 3).map((tag) => (
                        <span key={tag} className={`px-1.5 py-0 rounded border text-[10px] ${tagColors[tag] || "bg-secondary text-muted-foreground border-border"}`}>
                          {tag}
                        </span>
                      ))}
                  ...
                  <td className="px-3 py-2.5 text-green-700">{formatCost(m.cost_input)}</td>
                  <td className="px-3 py-2.5 text-green-700">{formatCost(m.cost_output)}</td>
                  <td className="px-3 py-2.5">
