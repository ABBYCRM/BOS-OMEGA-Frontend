...
// =====================================================================
// Task #84 — gpt-image-1 image-EDIT adapter.
//
// Hits `${base_url}/images/edits` with multipart/form-data. Unlike the
// generation endpoint, this one accepts an existing image (PNG or JPEG;
// PNG when transparency-preserving edits are needed) plus a free-text
// instruction and returns base64 PNG bytes the same way generation does.
//
// We deliberately do NOT pass a `mask` field: the user's intent is
// expressed as natural language ("make the sneaker blue") and the model
// edits the whole image. A future task can add mask support by extending
// the options struct without breaking this signature.
// =====================================================================
...
export async function callOpenAIImageEdit(
  ...
  try {
    const url = `${base_url.replace(/\/$/, "")}/images/edits`;
    // Normalise the source mime: gpt-image-1 accepts PNG/JPEG/WEBP and
    // rejects everything else with a 400. We pass the original mime
    // through; the upstream caller is responsible for ensuring source
    // bytes were originally generated by an image adapter (which only
    // ever emits image/png or image/jpeg).
    const ext = image_mime === "image/jpeg" ? "jpg"
      : image_mime === "image/webp" ? "webp"
      : "png";
    const file_blob = new Blob([new Uint8Array(image_bytes)], { type: image_mime });
    const form = new FormData();
    form.append("model", model);
    form.append("prompt", prompt);
    form.append("size", size);
    form.append("n", "1");
    form.append("image", file_blob, `source.${ext}`);

    const response = await fetch(url, {
      method: "POST",
      headers: {
        // IMPORTANT: do NOT set Content-Type — the runtime sets the
    ...
    if (response.status === 401 || response.status === 403) {
      return errResult(provider, model, start, "auth_failure", `OpenAI image-edit auth failure: ${response.status}`);
    }
    if (response.status === 429) {
      return errResult(provider, model, start, "rate_limit", "OpenAI image-edit rate limit");
    }
    if (response.status === 503) {
      return errResult(provider, model, start, "provider_outage", "OpenAI image-edit service unavailable");
    }
    if (response.status === 400) {
      const body_text = await response.text().catch(() => "");
      if (/safety|content[_ ]?policy|moderation/i.test(body_text)) {
    ...
    const b64 = data?.data?.[0]?.b64_json;
    if (!b64) {
      return errResult(provider, model, start, "malformed_response", "OpenAI image-edit response missing b64_json payload");
    }
    ...
    const error = err as Error;
    logger.error({ err, provider, model }, "OpenAI image-edit call failed");
    if (error?.name === "TimeoutError") {
      return errResult(provider, model, start, "timeout", "OpenAI image-edit request timed out");
    }
    return errResult(provider, model, start, "unknown_exception", error?.message ?? String(err));
  }
}
