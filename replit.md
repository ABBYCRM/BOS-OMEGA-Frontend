...
- `OPENAI_API_KEY` — required for Whisper audio/video transcription (otherwise honestly `skipped`). The Replit OpenAI proxy is also accepted as a fallback.
- `UPLOADS_DIR` — override storage root (defaults to `./.data/uploads`)
- `MAX_UPLOAD_BYTES` — override 50MB upload cap

### Replit AI Integrations proxy (no-key real LLM access)

When the workspace has the OpenAI / Anthropic / Gemini integrations provisioned, `AI_INTEGRATIONS_OPENAI_BASE_URL` + `AI_INTEGRATIONS_OPENAI_API_KEY` (and the matching pairs for Anthropic and Gemini) are auto-set. `lib/keyResolver.ts` honors them as the fourth fallback (after DB key, `api_key_env`, and the canonical `OPENAI_API_KEY`/`ANTHROPIC_API_KEY`/`GEMINI_API_KEY` env). When this fallback is used, the resolver returns `source: "proxy"` and a `base_url`, which the OpenAI / Anthropic / Gemini adapters and the Whisper transcription module honor instead of the hardcoded vendor URL. Charges go to the Replit account's credits — no per-vendor key required, no signup. Mock mode is only entered when none of the four sources yield a key.

## Recent changes

- **2026-04-26 — BOIL_THE_OCEAN-grade proxy fix (all 5 modes, all 4 providers)**: the previous proxy wiring still failed for the BOIL_THE_OCEAN, SERIES_PASS, and synthesis/adversarial paths. Two root causes: (1) `bos/providerBridge.ts` (used by series_pass + BTO synthesis/adversarial) didn't thread the proxy `base_url` through to the adapters, so proxy keys were sent to vendor URLs and returned 401 `auth_failure`. (2) The Replit Anthropic & Gemini proxies use a different model-name and endpoint shape than the public APIs. Fixed: `providerBridge.ts` now destructures `{key, base_url}` from `resolveProviderKey` and passes `base_url` as the 6th positional arg to `callOpenAI/Anthropic/Gemini` (matching `executionEngine.ts`). Renamed all Anthropic/Gemini model rows in `seed.ts` and the live DB to the proxy-supported IDs (`claude-sonnet-4-6`, `claude-haiku-4-5`, `claude-opus-4-7`; `gemini-2.5-pro`, `gemini-2.5-flash`) and added `multimodal` to all renamed Anthropic models. Updated the cost table in `anthropicAdapter.ts` to cover both new and legacy IDs. Refactored `geminiAdapter.ts` so the caller-supplied `base_url` IS the full API prefix (vendor: `…/v1beta`; proxy: bare host) — the adapter always appends `/models/{model}:generateContent` instead of hardcoding `/v1beta/`. End-to-end verified through the proxy with NO vendor key: single (HOLD on "Hello!"), parallel ("Paris"), consensus ("Tokyo"), series_pass ("Titan"), and boil_the_ocean (25/25 agents COMPLETED across OpenAI/Anthropic/Gemini, Ollama returns the expected `provider_outage`; final answer covers Einstein/Newton/Curie with adversarial finding folded into uncertainties). Cleared the stale `degraded` provider status row left over from the earlier failed runs.
- **2026-04-26 — Replit AI Integrations proxy as a no-key fallback**: provisioned the OpenAI, Anthropic, and Gemini Replit integrations. Extended `keyResolver.resolveProviderKey` to fall back to `AI_INTEGRATIONS_*_API_KEY` + `AI_INTEGRATIONS_*_BASE_URL` after DB / `api_key_env` / canonical-env lookups, returning `source: "proxy"` with a `base_url`. Threaded `base_url` as a 6th positional arg through `callOpenAI` / `callAnthropic` / `callGemini` (defaulting to the vendor URL when omitted) and wired `executionEngine.callProvider` to pass it. `transcription.ts` now also honors the proxy. Switched the Gemini adapter from `?key=` query auth to `x-goog-api-key` header so the proxy works. Updated `gpt-4o-mini` seed + live DB row to include `multimodal` capability tag (it really does support vision). Verified end-to-end: real text answer ("The capital of France is Paris.") and real vision answer ("The image shows a red square.") via gpt-4o-mini through the proxy with NO vendor key configured.
